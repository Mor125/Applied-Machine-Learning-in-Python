...
def blight_model():
    ...
    #relevant columns
    X = train[['fine_amount', 'late_fee', 'discount_amount', 'clean_up_cost', 'judgment_amount', 'lat', 'lon']]
    y = train['compliance']
    test = test[['fine_amount', 'late_fee', 'discount_amount', 'clean_up_cost', 'judgment_amount', 'lat', 'lon']]
    
    X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=0)
    
    clf = GradientBoostingClassifier(learning_rate = 0.5, max_depth = 50, random_state = 0)

    clf.fit(X_train, y_train)
    predicted_probs = clf.predict_proba(test)[:, 1]
    ans = pd.Series(predicted_probs, index=test.index)
